...
OPTIONS_MIN_RVOL            = float(os.getenv("OPTIONS_MIN_RVOL", "3.0"))         # min relative volume for MomentumCall entry (raised from 2.0)
OPTIONS_MIN_ADV             = float(os.getenv("OPTIONS_MIN_ADV", "2_000_000"))    # min 20-day avg daily dollar volume ($2M quality gate)
...
SWAP_ON_FULL         = False  # disabled — add new positions without swapping out existing ones
